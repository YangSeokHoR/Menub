...
- **매 실행마다 매니페스트 기록**: `writeManifest()`는 최초 1회가 아니라 **앱이 실행될 때마다** 최신본으로 덮어쓴다. 허브 설정에서 도구를 **삭제(일회성 제거)해도, 그 앱을 다시 실행하면 스스로 재등록**되어 후보로 다시 나타난다. (매니페스트를 한 번만 쓰도록 만들면 이 재등록이 깨진다)
...
- [ ] 허브 설정에서 **삭제** → 목록에서 사라짐, 그 앱 **재실행 시 후보로 재등장** (매 실행 재기록 확인)
